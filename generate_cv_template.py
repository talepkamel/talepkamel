#!/usr/bin/env python3
"""Generate a BLANK, fillable 3-page CV template (.docx) reproducing the
two-column design. All content is placeholder text in [brackets] that the
user can replace with their own information."""
...
# ---- Neutral circular photo placeholder ------------------------------------
...
sb_heading("Coordonnées")
sb_text("Tél : [ Téléphone ]", after=6)
sb_text("[ adresse@email.com ]", after=6)
sb_text("[ Adresse, Ville ]", after=6)

sb_heading("Langues")
p = sidebar.add_paragraph(); space(p, before=2, after=1)
add_run(p, "[ Langue 1 ]", size=11, color=WHITE)
progress_bar(sidebar, 0.8)
p = sidebar.add_paragraph(); space(p, before=4, after=1)
add_run(p, "[ Langue 2 ]", size=11, color=WHITE)
...
for _ in range(3):
    sb_text("[ Centre d'intérêt ]")


# ===== MAIN COLUMN (PAGE 1) ==================================================
p = main.paragraphs[0]
space(p, before=0, after=0)
add_run(p, "[ PRÉNOM NOM ]", size=30, bold=True, color=NAVY, font=HEAD_FONT)
p = main.add_paragraph(); space(p, before=2, after=18)
add_run(p, "[ Intitulé du poste ]", size=16, color=LIGHT_BLUE, caps=True,
        font=HEAD_FONT)
...
entry("[ Diplôme / Formation ]", "[ Établissement - Ville ]", "[ Années ]")
entry("[ Diplôme / Formation ]", "[ Établissement - Ville ]", "[ Années ]")

section_title("Expérience Professionnelle")
entry("[ Poste occupé ]", "[ Entreprise - Ville ]", "[ Dates ]")
bullet("[ Responsabilité ou réalisation principale ]")
bullet("[ Responsabilité ou réalisation principale ]")
bullet("[ Responsabilité ou réalisation principale ]")
entry("[ Poste occupé ]", "[ Entreprise - Ville ]", "[ Dates ]")
bullet("[ Responsabilité ou réalisation principale ]")
...
def banner():
    ...
    add_run(p, "[ PRÉNOM NOM ]", size=18, bold=True, color=WHITE, font=HEAD_FONT)
    p2 = c.add_paragraph(); space(p2, before=0, after=0)
    add_run(p2, "[ Intitulé du poste ]", size=10.5, color=WHITE, caps=True,
            font=HEAD_FONT)
    no_borders(t)
...
doc.add_page_break()
banner()
d_title("Profil Professionnel")
d_para("[ Rédigez ici un court paragraphe de présentation : votre profil, "
       "vos années d'expérience, vos points forts et votre objectif "
       "professionnel. ]")
d_title("Domaines d'Expertise")
for _ in range(5):
    d_bullet("[ Domaine d'expertise / savoir-faire ]")
d_title("Projets Marquants")
d_entry("[ Nom du projet ]", "[ Entreprise - Ville ]", "[ Dates ]")
d_bullet("[ Description / résultat du projet ]")
d_bullet("[ Description / résultat du projet ]")
d_entry("[ Nom du projet ]", "[ Entreprise - Ville ]", "[ Dates ]")
d_bullet("[ Description / résultat du projet ]")
d_bullet("[ Description / résultat du projet ]")
...
for _ in range(3):
    d_bullet("[ Certification - Organisme - Année ]")
d_title("Formations Complémentaires")
d_entry("[ Formation / Atelier ]", "[ Établissement - Ville ]", "[ Année ]")
d_bullet("[ Contenu / compétences acquises ]")
d_entry("[ Formation / Atelier ]", "[ Établissement - Ville ]", "[ Année ]")
d_bullet("[ Contenu / compétences acquises ]")
d_title("Réalisations Clés")
for _ in range(3):
    d_bullet("[ Réalisation ou résultat mesurable ]")
d_title("Références")
d_label("[ Nom du contact ]", "[ Poste, Entreprise ]")
d_label("Contact", "[ Téléphone · email ]")
d_para(" ")
d_label("[ Nom du contact ]", "[ Poste, Entreprise ]")
d_label("Contact", "[ Téléphone · email ]")

doc.save(OUT)
print("Saved", OUT, "(3-page blank template)")
