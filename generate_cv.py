...
# ---- Palette ----------------------------------------------------------------
NAVY = RGBColor(0x12, 0x32, 0x5A)        # dark sidebar / headings
NAVY_HEX = "12325A"
WHITE = RGBColor(0xFF, 0xFF, 0xFF)
LIGHT_BLUE = RGBColor(0x2E, 0x5E, 0x9E)  # accent text on white
GREY_BAR = "D9DEE6"
...
def progress_bar(cell, filled_ratio):
    ...
    set_cell_bg(left, "FFFFFF")
    set_cell_bg(right, "3A5A86")
    for c in (left, right):
...
def sidebar_heading(text):
    ...
    add_run(p, text, size=15, bold=True, color=WHITE, caps=True)
...
# Photo placeholder (rounded look approximated by a circle of text removed) ---
photo_p = sidebar.paragraphs[0]
photo_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
space(photo_p, before=0, after=6)
# Try to embed a real placeholder image if one exists; otherwise leave space.
import os
if os.path.exists("photo.png"):
    photo_p.add_run().add_picture("photo.png", width=Inches(1.7))
else:
...
add_run(p, "SACHA DUBOIS", size=34, bold=True, color=NAVY)

p = main.add_paragraph()
space(p, before=2, after=18)
add_run(p, "CHARGÉE DE PROJET", size=16, color=LIGHT_BLUE, caps=True)


def section_title(text):
    p = main.add_paragraph()
    space(p, before=10, after=10)
    add_run(p, text, size=18, bold=True, color=NAVY, caps=True)
...
def entry(title, org, date, *, org_italic=True):
    ...
    add_run(p, title, size=12.5, bold=True, color=NAVY)
